...
GuiGeophonino: The user interface developed by using Processing software.
...
void setup() {
  ...
  .setPosition(365,255).setColorValue(0).setFont(fontTex); 
  
  ddlGainLtb = cp5.addDropdownList("gainLtb").setPosition(430, 270);
 ...
 cp5.addTextlabel("AcquireData").setText("3º Data Acquisition")
  ...
  cp5.addButton("STOP Acquisition")
     .setValue(0)
     .setPosition(600,360)
     .setSize(100,14)
     .setColorBackground(color(255,51,0))
     .setId(8);   
...
void draw() {
  ...
  switch(state)
  {
    ...
    case(1)://1º Configure Connection
      ...
      cp5.controller("STOP Acquisition").setVisible(false);
    break;
    ...
    case(4)://4º Acquiring data from sensor
      ...
      fill(0,0,0);
      text(count-ini+"/"+duration+"sec.",240,370);
      text("<-Data acquisition progress",370,370);
      noFill();
      stroke(0);
      rect(160,355,200,19);
       cp5.controller("New Data Acquisition").setVisible(true);
       cp5.controller("STOP Acquisition").setVisible(true);
   break;
            ...
            if(gainSel<1)
              ...
              status.setValue("You must select sample rate");
                ...
                else //All parrameters are OK, send configuration to Geophonino already
                {
                  ...
                  String configComand="conf_adq" + "&nomF="+fileName+"&overW="+overwriteFile+"&Gain="+gainSel+"&tR="+duration+"&sR="+srSel+"&GLtb="+gainLtb+"\n";
                  print(configComand);
